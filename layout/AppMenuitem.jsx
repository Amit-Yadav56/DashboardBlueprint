...
const AppMenuitem = (props) => {
    ...
    const isActiveRoute = item && item.to && pathname === item.to;
    const active = activeMenu === key || activeMenu.startsWith(key + '-');
    const onRouteChange = (url) => {
        if (item && item.to && item.to === url) {
            setActiveMenu(key);
        }
    };
    ...
    const itemClick = (event) => {
        ...
        // toggle active state
        if (item && item.items) setActiveMenu(active ? props.parentKey : key);
        else setActiveMenu(key);
    };

    const subMenu = item && item.items && item.visible !== false && (
